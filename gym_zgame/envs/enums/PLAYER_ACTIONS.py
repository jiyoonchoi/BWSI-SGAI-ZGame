...
class DEPLOYMENTS(IntEnum):
    ...
    FLU_VACCINE_OPT = 7
    FLU_VACCINE_MAN = 8
    FLU_MUTATION_CENTER = 9
    KILN_OVERSIGHT = 10
    KILN_NO_QUESTIONS = 11
    BROADCAST_DONT_PANIC = 12
    BROADCAST_CALL_TO_ARMS = 13
    SNIPER_TOWER_CONFIRM = 14
    SNIPER_TOWER_FREE = 15
    PHEROMONES_BRAINS = 16
    PHEROMONES_MEAT = 17
    BSL4LAB_SAFETY_ON = 18
    BSL4LAB_SAFETY_OFF = 19
    RALLY_POINT_OPT = 20
    RALLY_POINT_FULL = 21
    FIREBOMB_PRIMED = 22
    FIREBOMB_BARRAGE = 23
    SOCIAL_DISTANCING_SIGNS = 24
    SOCIAL_DISTANCING_CELEBRITY = 25
    POLICE_STATION = 26
    SPY_INFORMANTS = 27
    ...
    def get_value_from_string(deployment):
        ...
        elif deployment.upper() == 'FLU_MUTATION_CENTER':
            return DEPLOYMENTS.FLU_MUTATION_CENTER.value
        ...
        elif deployment.upper() == 'POLICE_STATION':
            return DEPLOYMENTS.POLICE_STATION.value
        elif deployment.upper() == 'SPY_INFORMANTS':
            return DEPLOYMENTS.SPY_INFORMANTS.value
        else:
            warnings.warn(
                'Tried to convert string ({}) to DEPLOYMENTS enum and failed; returned NONE'.format(deployment))
            return DEPLOYMENTS.NONE.value
    ...
    def get_name_from_string(deployment):
        if deployment.upper() == 'NONE':
            ...
            return DEPLOYMENTS.FLU_MUTATION_CENTER.name
        elif deployment.upper() == 'KILN_OVERSIGHT':
            ...
            return DEPLOYMENTS.POLICE_STATION.name
        elif deployment.upper() == 'SPY_INFORMANTS':
            return DEPLOYMENTS.SPY_INFORMANTS.name
        else:
            warnings.warn(
                'Tried to convert string ({}) to DEPLOYMENTS enum and failed; returned NONE'.format(deployment))
            return DEPLOYMENTS.NONE.name
