...
class Neighborhood:
    ...
    def add_trust_level(self, n):
        self.set_trust_level(self.get_trust_level() + n)

    # get string representation of trust levels
    def get_trust_string(self):
        if self.get_trust_level() <= 15:
            return "REBELLIOUS"
        elif self.get_trust_level() <= 38:
            return "PANICKED"
        elif self.get_trust_level() <= 62:
            return "Nuetral"
        elif self.get_trust_level() <= 85:
            return "Compliant"
        else:
            return "Patriotic"

    def get_trust_number(self):
        if self.get_trust_level() <= 15:
            return 0
        elif self.get_trust_level() <= 38:
            return 1
    ...
    def mask_information(self, value):
        # Don't report out (to user and in state) the actual values, instead, bin them into a range
        trust_level = self.get_trust_number()
        if trust_level == 0:
            low = 0
            high = 0
        elif trust_level == 1:
    ...
    def raw_information(self):
        if self.spies_deployed or self.trust >= 90:
            data = self.get_data()
            return data['num_active', 'num_sickly', 'num_zombie', 'num_dead']
